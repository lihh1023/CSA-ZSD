...
def create_generator(args):
    if args.dataset_type == 'coco':
        ...
        validation_generator = CSVGenerator(
            args.annotations,
            args.classes,
            word_data_type=args.word_data_type,
        )
    else:
...
def parse_args(args):
    ...
    parser.add_argument('--word-data-type', help='Word data type.', type=str, default='word_coco_all')
...
def main(args=None):
    ...
    if args.word_data_type == 'word_coco_all':
        word_number = 80
        word_shape = 300
    else:
        # word_data = None
        print("word_data_type is false!")

    word_data_shape = (word_number, word_shape)
    ...
    attribute_shape = keras.layers.Input(shape=word_data_shape)
    inputs = [inputs_images, attribute_shape]
    resnet = keras_resnet.models.ResNet50(inputs[0], include_top=False, freeze_bn=True)
    model = retinanet.retinanet_bbox(inputs=inputs, num_classes=80, attribute_shape=word_data_shape, backbone=resnet)
    ...
    print('Unseen mAP: {:.4f} '.format(unseenMAP))
    print('Unseen Rec: {:.4f} '.format(unseenRE))
