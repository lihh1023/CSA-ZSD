...
def _get_detections(generator, model, num_seen, max_detections=100, save_path=None):
    ...
    for i in range(generator.size()):
        ...
        word = generator.word_data()

        # run network
        _, _, _, detections = model.predict_on_batch([np.expand_dims(image, axis=0), np.expand_dims(word, axis=0)])
        # clip to image shape
...
def evaluate(
    ...
    all_detections = _get_detections(generator, model, num_seen, max_detections=max_detections, save_path=save_path)
    ...
    for label in range(generator.num_classes()):
        ...
        if len(recall) != 0:
            average_recalls[label] = np.mean(recall)
