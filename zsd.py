...
from keras_retinanet.models import retinanet_san as retinanet
...
attribute_shape = keras.layers.Input(shape=(80, 300))
inputs = [inputs_images, attribute_shape]
resnet = keras_resnet.models.ResNet50(inputs[0], include_top=False, freeze_bn=True)
model = retinanet.retinanet_bbox(inputs=inputs, num_classes=80, attribute_shape=(80, 300), backbone=resnet)
model.load_weights(r'snapshots/gcn_0.001/gcn_seed_2_fl_contrastive_0.001/resnet50_csv_30.h5')

lines = open(r'val_picture.txt').read().split("\n")
num_rois = 100
visualise = False  # False True
detect_type = 'zsd'  # gzsd or zsd or seen_detection
seen_threshold = .4
unseen_threshold = .3
...
for idx in range(int(len(lines)) - 1):
    ...
    indices_seen = []
    tra = np.max(scores[:, :num_seen], axis=1)
    tra_arg = np.argmax(scores[:, :num_seen], axis=1)
    pos = np.where(tra > seen_threshold)
    indices_seen.append(pos[0])
    indices_seen.append(tra_arg[pos[0]])
    indices_seen = tuple(indices_seen)
    indices_unseen = []
    val = np.max(scores[:, num_seen:], axis=1)
    val_arg = np.argmax(scores[:, num_seen:], axis=1)
    ...
    cv2.imwrite('./Dataset/pro/{}'.format(filepath[49:]), img)
